...
const MIN_ASSET_DURATION_FRAMES = 6;
...
export const formatDurationFromFrames = (
...
export const createEditorAssetItem = async (file: File): Promise<EditorAssetItem> => {
...
export const createAssetDragItem = (asset: EditorAssetItem): SidebarTimelineItem => {
  ...
  const durationFrames = asset.durationSeconds
    ? Math.max(Math.round(asset.durationSeconds * DEFAULT_FRAME_RATE), MIN_ASSET_DURATION_FRAMES)
    : DEFAULT_CLIP_DURATION_FRAMES;
